...
@section('content_page')
    <div class = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-2 pt-12 pr-12 pl-12 pb-8 mx-auto">
        ...
        @foreach ($services as $service)
            <a href="/services/{service_id}">
                <div
                    class="mx-auto flex flex-col justify-center items-center w-full bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 p-6 shadow hover:shadow-xl">
                    <div class="flex flex-col items-center justify-center h-full">
                        <img class="h-24 mb-3 shadow-lg" src="/images/{{ $service->image }}" alt="{{ $service->image }}" />
                        <h5 class="mb-1 text-2xl font-bold text-sky-400 text-center">{{ $service->name }}</h5>
                    </div>
                </div>
            </a>
        @endforeach
